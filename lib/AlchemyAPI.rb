...
class AlchemyAPI_TargetedSentimentParams < AlchemyAPI_Params
  ...
  def getParameterString()
    retString = super()
    if (target != "")
      retString += "&target=" + URI.escape(@target)
    end
    if (showSourceText != "")
      retString += "&showSourceText=" + URI.escape(@showSourceText.to_s())
    end
...
class AlchemyAPI
  ...
  def URLGetAuthor(url, outputMode= OutputMode::XML,
                                baseParams = nil)
    ...
    DoGet("URLGetAuthor", "url", baseParams);
  rescue => err
    raise "Error making API call: #{err}"
  end

 def HTMLGetAuthor(html, url, outputMode= OutputMode::XML,
                                baseParams = nil)
    ...
    DoPost("HTMLGetAuthor", "html", baseParams);
  rescue => err
    raise "Error making API call: #{err}"
  end
  ...
  def TextGetTargetedSentiment(text, target, outputMode = OutputMode::XML,
			targetedSentimentParams = nil)
   CheckText(text, outputMode)

    if (targetedSentimentParams != nil && targetedSentimentParams.class != AlchemyAPI_TargetedSentimentParams &&
        targetedSentimentParams.class != AlchemyAPI_Params)
      raise "Invalid parameters object."
    end
    if (targetedSentimentParams == nil)
      targetedSentimentParams = AlchemyAPI_TargetedSentimentParams.new()
    end
    targetedSentimentParams.setText(text)
    targetedSentimentParams.setTarget(target)
    targetedSentimentParams.setOutputMode(outputMode)

    DoPost("TextGetTargetedSentiment", "text", targetedSentimentParams);
  rescue => err
    raise "Error making API call: #{err}"
  end

  def URLGetTargetedSentiment(url, target, outputMode = OutputMode::XML,
			targetedSentimentParams = nil)
   CheckURL(url, outputMode)
    ...
    DoGet("URLGetTargetedSentiment", "url", targetedSentimentParams);
  rescue => err
    raise "Error making API call: #{err}"
  end
  
  def HTMLGetTargetedSentiment(html, url, target, outputMode = OutputMode::XML,
			targetedSentimentParams = nil)
   CheckHTML(html, url, outputMode)
    ...
    DoPost("HTMLGetTargetedSentiment", "html", targetedSentimentParams);
  rescue => err
    raise "Error making API call: #{err}"
  end
